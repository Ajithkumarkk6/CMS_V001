...
class GetSectorServiceTypes(APIView):
...
class AddBrandCategory(APIView):
    ...
    def post(self, request, format=None):
        ...
        validateRequest = ValidateRequest(
            request=request, request_serializer=AddBrandCategorySerializer)
        ...
        if 'id' in data:
            update_model_response = create_update_model(BrandCategory, data, unique_fields=[
                                                        'name'], unique_q_extra_pram={"brand__id": employeeCompanyInfo.company.id})
        ...
        if 'parent_id' in data and data['parent_id'] is not None:
            parentBrandCategory = BrandCategory.objects.get(
                id=data['parent_id'])
        ...
        '''
        companyBranchInfos = CompanyBranchInfo.objects.filter(
            company=employeeCompanyInfo.company)
        ...
        for companyBranchInfo in companyBranchInfos:
            ...
            brandBranchCategory = BrandBranchCategory.objects.create(
                **brandBranchCategoryForm)
...
class AddBrandService(GenericAPIView):
    ...
    def proceed_post(self, request):
        ...
        brandServices = create_update_model(
            model_class=BrandServices, data=brandServiceForm)
        ...
        for companyBranchInfo in companyBranchInfos:
            ...
            create_brand_branch_service(
                companyBranchInfo, brandServices, create_brand_branch_service_payload)
    ...
    def post(self, request, format=None):
        ...
        if validation.is_valid():
            ...
            for service_id in service_ids:
                ...
                serializer = PredefinedProductSerializer(
                    predefinedCategorys, many=True)
                response_data.append(
                    {"id": service_id, "data": serializer.data})
    ...
    def post(self, request, format=None):
        ...
        if validation.is_valid():
            ...
            for uom in data['uoms']:
                ...
                try:
                    brandServiceMasterUomFormFieldForm['field_type'] = FormFieldType.objects.get(
                        tag=uom['uom_field_type'])
                except:
                    brandServiceMasterUomFormFieldForm['field_type'] = FormFieldType.objects.get(
                        tag='TEXT')

                brandServiceMasterUomFormFieldForm['is_display'] = uom['is_display']
                brandServiceMasterUomFormField = BrandServiceMasterUomFormField.objects.create(
                    **brandServiceMasterUomFormFieldForm)
                ...
                brandServiceMasterUomFormFieldServiceMappingForm[
                    'master_uom'] = brandServiceMasterUomFormField
                brandServiceMasterUomFormFieldForm['brand'] = companyMeta

                BrandServiceMasterUomFormFieldServiceMapping.objects.create(
                    **brandServiceMasterUomFormFieldServiceMappingForm)
...
class SubmitBrandServices(APIView):
    ...
    def post(self, request, format=None):
        ...
        if validation.is_valid():
            ...
            companyBranchInfo = CompanyBranchInfo.objects.filter(
                company=companyMeta).first()
            ...
            for service_category in service_categorys:
                if service_category['is_parent'] == True:
                    ...
                    if 'reference_id' in service_category:
                        try:
                            predefinedCategory = PredefinedCategory(
                                id=service_category['reference_id'])
                    ...
                    brandCategory = BrandCategory.objects.create(
                        **brandCategoryForm)
                    ...
                    brandBranchCategory = BrandBranchCategory.objects.create(
                        **brandCategoryForm)
                    brand_branch_category_mapping_ids.append(
                        {"brand_category_id": brandCategory.id, "brand_branch_category_id": brandBranchCategory.id})
            ...
            for brand_service in brand_services:
                ...
                brandServices = BrandServices.objects.create(
                    **brandServicesForm)
        ...
        if validation.is_valid():
            ...
            for week_calendar in data['week_calendar']:
                if week_calendar['day'] == 1:
                    companyBranchWeeklyCalendarForm['day_mon'] = createWeekDay(
                        week_calendar)
                elif week_calendar['day'] == 2:
                    companyBranchWeeklyCalendarForm['day_tue'] = createWeekDay(
                        week_calendar)
                elif week_calendar['day'] == 3:
                    companyBranchWeeklyCalendarForm['day_wed'] = createWeekDay(
                        week_calendar)
                elif week_calendar['day'] == 4:
                    companyBranchWeeklyCalendarForm['day_thu'] = createWeekDay(
                        week_calendar)
                elif week_calendar['day'] == 5:
                    companyBranchWeeklyCalendarForm['day_fri'] = createWeekDay(
                        week_calendar)
                elif week_calendar['day'] == 6:
                    companyBranchWeeklyCalendarForm['day_sat'] = createWeekDay(
                        week_calendar)
                elif week_calendar['day'] == 7:
                    companyBranchWeeklyCalendarForm['day_sun'] = createWeekDay(
                        week_calendar)

            companyBranchWeeklyCalendarForm['company_branch_id'] = request_info['company_branch']['id']
            CompanyBranchWeeklyCalendar.objects.create(
                **companyBranchWeeklyCalendarForm)
