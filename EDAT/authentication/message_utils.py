...
TEMP_REGISTRATION_SUCCESSFUL = {
    ...
    "content": "Thank you {#var#} {#var#} {#var#} for registering with Quantatm. You will be notified shortly upon approval."
}

TEMP_REGISTRATION_OTP = {
    "message_title": "Quantatm Register OTP",
    "purpose": "OTP",
    "sender_id": "ZENYLG",
    "template_id": 1107165709050778516,
    "content": "Your Verification OTP for Quantatm Registration is {#var#}"
}

TEMP_LOGIN_OTP = {
    "message_title": "Quantatm Login Otp",
    "purpose": "OTP",
    "sender_id": "ZENYLG",
    "template_id": 1107165709004657343,
    "content": "Your Login OTP for Quantatm is {#var#}"
}
...
TEMP_EMPLOYEE_ADDED = {
    ...
    "content": "You have been successfully registered with Quantatm by {#var#}. Download the Quantatm App in Play store/App store to login."
}
...
def prepare_request(template, mobile):
    url = 'https://api.k111aleyra.io/v1/HXIN1737577934IN/messages'
    payload = {"to": mobile, 'type': template['purpose'], 'sender': template['sender_id'],
               'template_id': template['template_id'], 'body': template['content']}
    headers = {'content-type': 'application/x-www-form-urlencoded',
               'Accept-Charset': '*/*', 'api-key': 'A80c7eaaa683c48ebc21dd7cdd49384caa29'}
    return {'url': url, 'headers': headers, 'payload': payload}


def send_company_registration_successful(name, mobile_number):
    data = prepare_request(TEMP_REGISTRATION_SUCCESSFUL,
                           COUNTRY_PREFIX_IN+mobile_number)
    ...
    try:
        ...
        print(data['headers'])
        r = requests.post(
            url=data['url'], data=data['payload'], headers=data['headers'])
        print("ererr")
        print(r)
...
def send_company_registration_OTP(name, mobile_number, otp):
    data = prepare_request(TEMP_REGISTRATION_OTP,
                           COUNTRY_PREFIX_IN+mobile_number)
    content = data['payload']['body']
...
def send_employee_registration(company_name, mobile_number):
    # "content":"You have been successfully registered with Quantatm by {#var#}. Download the Quantatm App in Play store/App store to login."
    data = prepare_request(TEMP_EMPLOYEE_ADDED,
                           COUNTRY_PREFIX_IN+mobile_number)
    content = data['payload']['body']
