...
from company.model_helper import get_all_branchs, get_brand_sectors, get_departments, get_type_of_business
...
from company.response_serializer import CompanyTypeOfBusinessSerializer, CompanySectorSerializer, DepartmentDropdownSerializer, BranchListSerializer
...
from .models import VehicleTypes, VehicleDetails
from .response_serializer import GetVehiclelistSerializer
    ...
    def post(self, request, format=None):
        ...
        if request_info['company_info'] is not None :
                ...
                if 'q' in data and branchQ.count()>0:
                    branchQ = branchQ.filter(Q(name__istartswith = data['q']) | Q(display_name__icontains = data['q']))
...
class GetDepartments(APIView):
    ...
    def post(self, request, format=None):
        ...
        request_info = get_user_company_from_request(request)
        if request_info['company_info'] is not None :
                departmentsQ = get_departments(request_info['company_info'].company.id)
...
class UpdateBranchLocation(APIView):
    ...
    def post(self, request, format=None):
        ...
        if validation.is_valid() and user['company_info'] is not None:                
            ...
            try:
                company_geolocation =  CompanyGeoLocationInfo.objects.get(id = data['id'])
    ...
    def post(self, request, format=None):
        ...
        if validation.is_valid() and user['company_info'] is not None:
            ...
            if user['company_info'].company_branch.company_geolocation is None:
                ...
                form_company_geolocation = {}
                form_company_geolocation = data
                company_geolocation = CompanyGeoLocationInfo(**form_company_geolocation, is_active=True)
    ...
    def post(self, request, format=None):
        ...
        if validation.is_valid() and request_info['company_info'] is not None:
            if check_if_exist(CompanyBranchInfo, {"name":data['name'], "company":request_info['company_info'].company}) == False:
                ...
                company_branch = CompanyBranchInfo(company_contact = company_contact, **form_company_branch)
...
class GetCustomerHomeContent(APIView):
    ...
    def post(self, request, format=None):
        ...
        response_data['base_content'] = {'footer':[{'order':1,'text':'Products'},{'order':2,'text':'Bookings'},{'order':3,'text':'Store'}]}
...
class GetStoreDetails(APIView):
    ...
    def post(self, request, format=None):
        ...
        if validation.is_valid() and request_info['company_info'] is not None:
            ...
            companyContactInfo = CompanyBranchInfo.objects.get(id=data['branch_id'])

            companyBranchWeeklyCalendar = CompanyBranchWeeklyCalendar.objects.filter(company_branch__id=data['branch_id']).first()

            
            week_days.append({"day":1,"is_working":companyBranchWeeklyCalendar.day_mon.is_working,"start_time":companyBranchWeeklyCalendar.day_mon.start_time,"end_time":companyBranchWeeklyCalendar.day_mon.end_time})
            week_days.append({"day":2,"is_working":companyBranchWeeklyCalendar.day_tue.is_working,"start_time":companyBranchWeeklyCalendar.day_tue.start_time,"end_time":companyBranchWeeklyCalendar.day_tue.end_time})
            week_days.append({"day":3,"is_working":companyBranchWeeklyCalendar.day_wed.is_working,"start_time":companyBranchWeeklyCalendar.day_wed.start_time,"end_time":companyBranchWeeklyCalendar.day_wed.end_time})
            week_days.append({"day":4,"is_working":companyBranchWeeklyCalendar.day_thu.is_working,"start_time":companyBranchWeeklyCalendar.day_thu.start_time,"end_time":companyBranchWeeklyCalendar.day_thu.end_time})
            week_days.append({"day":5,"is_working":companyBranchWeeklyCalendar.day_fri.is_working,"start_time":companyBranchWeeklyCalendar.day_fri.start_time,"end_time":companyBranchWeeklyCalendar.day_fri.end_time})
            week_days.append({"day":6,"is_working":companyBranchWeeklyCalendar.day_sat.is_working,"start_time":companyBranchWeeklyCalendar.day_sat.start_time,"end_time":companyBranchWeeklyCalendar.day_sat.end_time})
            week_days.append({"day":7,"is_working":companyBranchWeeklyCalendar.day_sun.is_working,"start_time":companyBranchWeeklyCalendar.day_sun.start_time,"end_time":companyBranchWeeklyCalendar.day_sun.end_time})

            response_data['week_days'] = week_days
            response_data['address_details'] = {"name":companyContactInfo.company_contact.communication_address, "place_id":companyContactInfo.company_contact.google_place_link}

            contact_number = []
            contact_number.append(companyContactInfo.company_contact.mobile_number_01)

            if companyContactInfo.company_contact.mobile_number_02 is not None:
                contact_number.append(companyContactInfo.company_contact.mobile_number_02)
...
class GetVehicleTypes(GenericListAPIView):

    
    access_rights = GenericAPIView.ACCESS_TYPE_EMPLOYEE
    request_serializer =  None
    list_serializer = GetVehiclelistSerializer

    def get_list_query(self):
            return VehicleTypes.objects.filter(company_branch = self.get_employee_company_info().company_branch)
