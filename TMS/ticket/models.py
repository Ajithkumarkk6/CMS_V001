...
class Ticket(BaseModelMixin):
    ...
    many_to_many_fields = ['tags','reference_ticket']

    def get_many_to_many_class(self, name):
        if name == 'tags':
            return TicketTag
        elif name=='reference_ticket':
            return Ticket
        return None
    ...
    reference_ticket = models.ManyToManyField('self',symmetrical = False,blank=True)
...
class TicketChannelEvent(BaseModelMixin):
    ...
    attachments = models.ForeignKey(TicketAttachmentMessage, on_delete=models.CASCADE, null=True, blank=True)
    assigned_to = models.ForeignKey(
