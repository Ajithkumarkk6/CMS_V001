...
class RegisterCompany(APIView):
    ...
    def post(self, request, format=None):
        ...
        form_company_meta['attachment_logo'] = data['attachment_logo']
    ...
    def post(self, request, format=None):
        ...
        if validation.is_valid():
            ...
            if user is not None:
                ...
                userCredentialValidation = UserCredentialValidation.objects.get(mobile_number=data['mobile_number'])
                # return Response(get_validation_failure_response(None, 'Invalid user'))
                if userCredentialValidation.mobile_otp == data['otp']:
                    userCredentialValidation.is_active = True
                    userCredentialValidation.is_otp_verified = True
                    userCredentialValidation.save()
                    return Response (get_success_response("User registered Successfully"))
                else:
                    return Response(get_validation_failure_response(None, 'Invalid Otp'))
        else:
            return Response(get_validation_failure_response(validation.errors))
