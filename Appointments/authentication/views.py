...
class DeleteCompany(APIView):
    ...
    def post(self, request):
        ...
        employeeCompanyInfos = EmployeeCompanyInfo.objects.filter(company__id = company_id)
...
class RegisterAdmin(APIView):
    ...
    def post(self, request, format=None):
        ...
        validateRequest  = ValidateRequest(request=request, request_serializer=RegisterUserSerializer)
        ...
        if validation.is_valid():
            ...
            return Response(get_success_response("Admin registered Successfully"))
        else:
...
class RegisterCustomer(APIView):
    ...
    def post(self, request, format=None):
        ...
        validateRequest  = ValidateRequest(request=request, request_serializer=RegisterCustomerSerializer)
        if not validateRequest.is_valid_open_request():
            return Response(get_validation_failure_response(None, validateRequest.errors_formatted()))
        ...
        if validation.is_valid():
            ...
            if 'branch_code' in data:
                try:
                    companyBranchInfo = CompanyBranchInfo.objects.get(code=data['branch_code'])
    ...
    # , department = employeeDepartment, designation = employeeDesignation, company=companyMeta, company_branch= company_branch,
                    EmployeeCompanyInfo.objects.create(user=user, company=companyBranchInfo.company, company_branch=companyBranchInfo, **form_employee_company_info)
                except:
                    pass
            set_mobile_otp(userAuthentication, is_login=False)
...
class AttachUserAccounts(APIView):
    ...
    def post(self, request, format=None):
        ...
        validateRequest  = ValidateRequest(request=request, request_serializer=AttachUserAccountsSerializer)
...
class RegisterCompany(APIView):
    ...
    def post(self, request, format=None):
        ...
        validateRequest  = ValidateRequest(request=request, request_serializer=RegisterCompanySerializer)
        if not validateRequest.is_valid_open_request():
            return Response(get_validation_failure_response(None, validateRequest.errors_formatted()))
        ...
        if validateRequest.is_valid():
            return Response(get_validation_failure_response(None, "Invalid Request. Company already exist"))
        elif validateRequest.is_admin() == False:
            return Response(get_validation_failure_response(None, "Action not permitted by user"))
        ...
        validation_form_company_meta = CompanyMetaSerializer(data=form_company_meta)               
        ...
        if validation_form_company_meta.is_valid():
            ...
            company_contact = CompanyContactInfo.objects.create(**form_company_contact)

            # adding company branch
            form_company_branch = {}
            form_company_branch['name'] = data['business_name']
            form_company_branch['display_name'] = data['registered_name']
            form_company_branch['is_parent'] = True
            form_company_branch['is_active'] = True

            company_branch = CompanyBranchInfo.objects.create(company_contact = company_contact,  company=companyMeta, **form_company_branch)
            
            user = get_user_from_request(request_info, data)
        
            userAuthentication = UserAuthentication.objects.get(user = user, is_admin=True)
            ...
            EmployeeCompanyInfo.objects.create(user=user, authentication = userAuthentication, department = employeeDepartment, designation = employeeDesignation, company=companyMeta, company_branch= company_branch, **form_employee_company_info)

            return Response(get_success_response("Registation Successful"))
...
class ValidateUser(APIView):
    ...
    def post(self, request, format=None):
        ...
        if validation_mobile.is_valid():
            ...
            if user is not None:
                try:
                    userAuthentication = UserAuthentication.objects.get(user = user, is_active=True)
            ...
            if user is not None:
                try:
                    ...
                    if userAuthentication.is_admin: 
                        ...
                        try:
                            EmployeeCompanyInfo.objects.get(user = userAuthentication.user, designation__is_admin=True)
                            set_mobile_otp(userAuthentication, is_login=True)
                            return Response(get_success_response("User Validated Successfully"))
                        except:
...
class GetBusinessPlaces(APIView):
    ...
    def post(self, request, format=None):
        ...
        if validation.is_valid():
            ...
            # from ttext
            if data['type'] == 'phone':
                list_google_places = 'https://maps.googleapis.com/maps/api/place/findplacefromtext/json?fields=formatted_address%2Cname%2Cplace_id&input=%2B91'+q+'&inputtype=phonenumber&country=in&key=AIzaSyCxiyjYNDLQ4D1HZkm4g4M0D6XEsDgbHHo&fields=formatted_address%2Cplace_id'
                ...
                for place in formatted_list:
                    response_data.append({"place_id":place['place_id'], 'title': place['name'], 'address': place['formatted_address']})
                return Response(get_success_response(None,None, response_data))

            elif data['type'] == 'address':
                list_google_places = 'https://maps.googleapis.com/maps/api/place/findplacefromtext/json?fields=formatted_address%2Cname%2Cplace_id&input='+q+'&inputtype=textquery&country=in&key=AIzaSyCxiyjYNDLQ4D1HZkm4g4M0D6XEsDgbHHo'
...
class GetBusinessPlaceDetails(APIView):
    ...
    def post(self, request, format=None):
        ...
        if validation.is_valid():
            ...
            try:
                place_id = data['id']
                r = requests.get(url='https://maps.googleapis.com/maps/api/place/details/json?place_id='+place_id+'&key=AIzaSyCxiyjYNDLQ4D1HZkm4g4M0D6XEsDgbHHo')
                print("dfsdfdsfdsfr")
                print(r.content)
                formatted_list = json.loads(r.content)
                ...
                if 'formatted_phone_number' in formatted_list:
                    response_data['contact_number'] = formatted_list['formatted_phone_number'].replace(" ", '')[-10:]
...
class DeleteUser(APIView):
    ...
    def post(self, request, format=None):
        ...
        if request_info['user'] is not None :
            try:
                userAuthentication = UserAuthentication.objects.get(user = request_info['user'], is_active=True)
            ...
            if user is not None:
                ...
                if userAuthentication is not None:
                    if userAuthentication.mobile_otp == data['otp']:
                        ...
                        response = {"success":True, "token":token, "message":"User Loggedin Successfully"}

                        user_company_info = get_user_company_from_user(user)
                        response['company'] = user_company_info['company']
...
class OtpRegister(APIView):
    ...
    def post(self, request, format=None):
        ...
        if validation.is_valid():
            userCredentialValidations =  UserCredentialValidation.objects.filter(mobile_number = data['mobile_number'])
            ...
            if user is not None:
                ...
                if current_time < otp_expiry:
                    if userAuthentication.mobile_otp == data['otp']:
                        ...
                        userAuthentication.is_admin = True
                        userAuthentication.save()
                        response = get_success_response("Admin registered Successfully")
...
class Dashboard(APIView):
    ...
    def post(self, request, format=None):
        ...
        if request_info['company_info'] is not None :
            ...
            current_date_info = get_current_start_end_time()
            eachUserLog = get_user_logs(request_info['user'], current_date_info['date_start'], current_date_info['date_end'])
            
            employeePersonalInfo = EmployeePersonalInfo.objects.get(user = request_info['user'])
            user_details["phone"] = employeePersonalInfo.mobile_number
            user_details["mobile_number"] = employeePersonalInfo.mobile_number
            user_details["gender"] = employeePersonalInfo.gender

            if employeePersonalInfo.attachment_profile and hasattr(employeePersonalInfo.attachment_profile, 'url'):
                user_details["profile_photo"] = employeePersonalInfo.attachment_profile.url
            else:
                user_details["profile_photo"] = None
            
            user_details["checkin_time"] = None

            user_details["checkout_time"] = None
            
            if eachUserLog is not None:
                if 'start_time' in eachUserLog:
                    user_details["checkin_time"] = eachUserLog['start_time']
                
                if 'end_time' in eachUserLog:
                    user_details["checkout_time"] = eachUserLog['end_time']
            ...
            permission_details["branch_id"] = company_branch.id
            permission_details["company_id"] = request_info["company_info"].company.id

            permission_details["is_fence_admin"] = False



            permission_details['has_company_branch_location'] = False
            permission_details['can_update_location'] = True
            
            associated_branches = []
            permission_details['associated_branches'] = associated_branches

            if company_branch.company_geolocation is not None:
                permission_details['has_company_branch_location'] = True
                company_geolocation = company_branch.company_geolocation 
                permission_details['company_branch_location'] = {'id':company_geolocation.id, "location_latitude":company_geolocation.location_latitude, "location_longitude":company_geolocation.location_longitude, 'fencing_radius': company_geolocation.fencing_radius}
            
            details = {"user_details":user_details, "permission_details":permission_details}
            details['company'] = request_info['company']
            details['company']['fencing_radius_options'] = [{"key":30, "value":"30"},{"key":50, "value":"50"},{"key":100, "value":"100"},{"key":150, "value":"150"},{"key":200, "value":"200"},{"key":500, "value":"500"},{"key":1000, "value":"1 Km"}]
            details['company_branch'] = request_info['company_branch']
            details['company_branch']['fencing_radius_options'] = [{"key":30, "value":"30"},{"key":50, "value":"50"},{"key":100, "value":"100"},{"key":150, "value":"150"},{"key":200, "value":"200"},{"key":500, "value":"500"},{"key":1000, "value":"1 Km"}]
