...
class GenericAPIView(APIView):
    ...
    def post(self, request):
        ...
        self.validateRequest = ValidateRequest(request=request, request_serializer=self.request_serializer)
...
class GenericCrudApiView(GenericAPIView):
    ...
    def proceed_post(self, request):        
        if self.base_model is not None:
            return create_update_model_response(self.base_model, self.payload)
        return Response(get_validation_failure_response(None, self.validateRequest.errors_formatted()))
    ...
    def proceed_post(self, request):
        if self.get_list_query() is not None and self.list_serializer is not None:
            ...
            if self.per_page_count == -1:  
                serializer = self.list_serializer(self.get_list_query(), many=True)
                ...
                res = get_paginated_results_set(self.get_list_query(), self.list_serializer, page_number, self.per_page_count)
